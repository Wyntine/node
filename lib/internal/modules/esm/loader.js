...
function getRaceMessage(filename, parentFilename) {
  ...
  raceMessage +=
    'import()-ed via Promise.all(). Try await-ing the import() sequentially in a loop instead.';
  if (parentFilename) {
...
class ModuleLoader {
  ...
  async executeModuleJob(url, wrap, isEntryPoint = false) {
    ...
    const module = await onImport.tracePromise(
      async () => {
   ...
   * @returns {Promise<ModuleJobBase>}
   */
  getModuleJobForRequireInImportedCJS(
    specifier,
    parentURL,
    importAttributes,
    phase
  ...
  importSyncForRequire(mod, filename, source, isMain, parent) {
    ...
    if (job !== undefined) {
      ...
      if (status === kEvaluated) {
        return {
          wrap: job.module,
          namespace: job.module.getNamespaceSync(filename, parentFilename),
        };
      ...
      if (status !== kEvaluating) {
        assert.fail(
          `Unexpected module status ${status}. ` +
  ...
  getModuleJobForRequire(specifier, parentURL, importAttributes, phase) {
    ...
    // loadSync() here.
    const resolveResult = this.#cachedResolveSync(
      specifier,
      parentURL,
      importAttributes
    );
    ...
    const finalFormat =
      loadResult.format === 'commonjs' ? 'commonjs-sync' : loadResult.format;
    ...
    assert(
      wrap instanceof ModuleWrap,
      `Translator used for require(${url}) should not be async`
    );
  ...
  #createModuleJob(
    ...
    if (isForRequireInImportedCJS) {
      moduleOrModulePromise = this.loadAndTranslateForRequireInImportedCJS(
        url,
        context,
        isMain
      );
  ...
  async import(
    ...
    return onImport.tracePromise(
      async () => {
        const moduleJob = await this.getModuleJobForImport(
          specifier,
          parentURL,
          importAttributes,
          phase
  ...
  resolve(specifier, parentURL, importAttributes) {
    ...
    if (this.#customizations) {
      // Only has module.register hooks.
  ...
  #resolveAndMaybeBlockOnLoaderThread(specifier, context) {
    ...
    return this.#cachedDefaultResolve(
      specifier,
      context.parentURL,
      context.importAttributes
    );
  ...
  importMetaInitialize(meta, context) {
    ...
    // Make cache available as import.meta.cache similar to import.cache
